...
const fallbackImage = "/default-product.png"; // in /public
...
export default function NewCollection() {
  ...
  const getIndex = (index) => (index + products.length) % products.length;

  const displayedProducts = Array.from({ length: visibleCount }, (_, i) =>
    products[getIndex(currentIndex + i)]
  );
              ...
              product && (
                ...
                >
                  <div className="relative w-full h-[300px] sm:h-[250px] overflow-hidden rounded-lg">
                    <Image
                      src={product.image ? `${imageBaseURL}${product.image}` : fallbackImage}
